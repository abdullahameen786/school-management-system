...
import imageCompression from 'browser-image-compression'; // 🚀 Client-side image compression
import { 
...
const StudentLayout = () => {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      ...
      // Sync plain password in Firestore for administrative reference logs
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-sky-950 text-sky-100 flex flex-col transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all ${
                  isActive 
                    ...
                    : 'text-sky-300/60 hover:bg-sky-900/50 hover:text-sky-100'
                }`}
              >
        ...
        {/* Bottom Core Actions */}
        <div className="p-4 shrink-0 border-t border-sky-900/40 bg-sky-950 space-y-1">
          <button 
            onClick={() => { setIsSettingsModalOpen(true); setActiveTab('profile'); setSidebarOpen(false); }}
            className="flex w-full items-center gap-3 px-4 py-3 text-sm font-semibold text-sky-300/60 hover:bg-sky-900/50 hover:text-sky-100 rounded-xl transition-colors cursor-pointer"
          >
            <Settings className="h-5 w-5 shrink-0 text-sky-400" />
            Account Settings
          </button>
        ...
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6 shadow-xs z-10">
          ...
          <div className="flex items-center gap-4 ml-auto">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-[11px] font-bold text-sky-600 tracking-wider truncate max-w-[160px]">{user?.email}</p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden transform transition-all animate-in zoom-in-95 duration-200">
            ...
            <div className="flex border-b border-slate-100">
              <button 
                ...
                className={`flex-1 py-3 text-sm font-semibold transition-colors cursor-pointer ${activeTab === 'profile' ? 'text-sky-600 border-b-2 border-sky-600 bg-sky-50/30' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}`}
              >
                Profile Picture
              </button>
              <button 
                onClick={() => setActiveTab('password')}
                className={`flex-1 py-3 text-sm font-semibold transition-colors cursor-pointer ${activeTab === 'password' ? 'text-sky-600 border-b-2 border-sky-600 bg-sky-50/30' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}`}
              >
                Security
              </button>
            </div>
                ...
                <form onSubmit={handleProfileUpdate} className="space-y-6 flex flex-col items-center">
                  <div className="relative group">
                    {avatarPreview ? (
                      ...
                      <div className="h-28 w-28 rounded-full bg-sky-50 border-4 border-slate-100 shadow-md flex items-center justify-center text-4xl font-bold text-sky-600 relative overflow-visible">
                        {displayName.charAt(0).toUpperCase()}
                      </div>
                    )}
                    ...
                    <label className="absolute bottom-0 right-0 p-2 bg-sky-600 text-white rounded-full shadow-lg hover:bg-sky-700 cursor-pointer transition-colors border-2 border-white z-10">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 text-sm font-semibold text-white bg-sky-600 hover:bg-sky-700 disabled:bg-sky-400 rounded-xl shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-2"
                  >
                    {avatarLoading ? (
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="block w-full rounded-xl border border-slate-200 bg-slate-50/50 py-2.5 pl-4 pr-10 text-slate-900 focus:border-sky-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-sky-500 sm:text-sm"
                      />
